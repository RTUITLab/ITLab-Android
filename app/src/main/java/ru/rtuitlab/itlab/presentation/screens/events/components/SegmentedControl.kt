...
@Preview
@Composable fun SegmentedPreview() {
	MaterialTheme {
		Surface {
...
private val BACKGROUND_SHAPE = RoundedCornerShape(5.dp)
...
fun SegmentText(
	...
	text: String,
	selected: Boolean,
	selectedColor: Color = AppColors.accent.value,
	unselectedColor: Color = AppColors.greyText.value,
...
private fun Thumb(state: SegmentedControlState) {
	val startCornerRadius by animateDpAsState(
		targetValue = if (state.selectedSegment == 0) 5.dp else 0.dp
	)
	val endCornerRadius by animateDpAsState(
		targetValue = if (state.selectedSegment == state.segmentCount - 1) 5.dp else 0.dp
	)
	Box(
		Modifier
			.then(
			...
			.background(
				AppColors.accent.collectAsState().value.copy(alpha = .15f),
				BACKGROUND_SHAPE
			)
			.border(
				width = 1.dp,
